...
let AuthController = class AuthController {
    ...
    signup(body) { return this.auth.signup(body.username, body.password, body.email); }
    login(body, res) { return this.auth.login(res, body.username, body.password); }
    refresh(req, res) {
        const user = req.user;
        return this.auth.refresh(res, user.sub, user.username, user.token);
    }
    logout(req, res) { return this.auth.logout(res, req.user.sub); }
    me(req) { const { sub, username } = req.user; return { id: sub, username }; }
};
